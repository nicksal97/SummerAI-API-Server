...
# NEW
import tempfile  # for safe temp dirs while handling uploads
...
# ---------- HELPER: pick a model file if model_name is not provided ----------
def _pick_model_file(model_selection: str, model_name_from_post: str | None) -> str:
    """
    Try to resolve the full path to a .pt model based on:
    - model_selection ('summer' or 'winter')
    - model_name_from_post (e.g., 'best.pt'); if missing, pick best/first .pt
    """
    if model_selection == "winter":
        model_dir = Path('static/models/germany_winter_ai_model')
    else:
        model_dir = Path('static/models/germany_summer_ai_model')

    model_dir.mkdir(parents=True, exist_ok=True)

    # If explicitly provided, use it
    if model_name_from_post:
        candidate = model_dir / model_name_from_post
        if candidate.exists():
    ...
    # Otherwise first .pt in the folder
...
def model_upload(request):
    """
    ENHANCED:
    - POST .pt file -> save under selected summer/winter folder (original behavior)
    - POST ZIP with .pt -> extract and save models
    - POST ZIP with images (jpg/jpeg/png/tif/tiff) -> run the same inference flow as 'index'
      and return JSON with geojson/zip/result paths (no frontend change needed)
    - GET -> list available models (original behavior)
    """
    if request.method == 'POST':
        # Accept file under multiple common field names
        upload = request.FILES.get('model_path') or request.FILES.get('file') or request.FILES.get('image') or request.FILES.get('upload')
        if not upload:
            return JsonResponse({"status": False, "message": "No file uploaded (expected 'model_path'/'file'/'image'/'upload')."}, status=400)

        model_selection = request.POST.get('model', 'summer')  # 'summer' or 'winter'
        model_name = request.POST.get('model_name')  # may be None for image ZIP uploads

        # CASE A: single .pt model file
        if upload.name.lower().endswith('.pt'):
            ...
            location.save(upload.name, upload)
            context = {
                "status": True,
        ...
        # Save upload temporarily (ZIPs handled here)
        ...
        if upload.name.lower().endswith('.zip'):
            try:
                with zipfile.ZipFile(tmp_path, "r") as zf:
                    ...
                    has_pt = any(Path(n).suffix.lower() == ".pt" for n in names)
                    has_img = any(Path(n).suffix.lower() in [".jpg", ".jpeg", ".png", ".tif", ".tiff"] for n in names)
            except zipfile.BadZipFile:
                shutil.rmtree(tmp_dir, ignore_errors=True)
                return JsonResponse({"status": False, "message": "Uploaded file is not a valid ZIP."}, status=400)

            # CASE B: model ZIP (contains .pt) -> extract to models dir (summer/winter)
            if has_pt and not has_img:
                if model_selection == "summer":
                    dest_dir = Path('static/models/germany_summer_ai_model')
                else:
                    dest_dir = Path('static/models/germany_winter_ai_model')
                dest_dir.mkdir(parents=True, exist_ok=True)
                with zipfile.ZipFile(tmp_path, "r") as zf:
                    for nm in names:
                        if Path(nm).suffix.lower() == ".pt":
                            # maintain file name under chosen model dir
                            out_path = dest_dir / Path(nm).name
                            with zf.open(nm) as src, open(out_path, "wb") as dst:
                                shutil.copyfileobj(src, dst)
                shutil.rmtree(tmp_dir, ignore_errors=True)
                return JsonResponse({"status": True, "message": "Model Upload Successfully (ZIP extracted)."})
            
            # CASE C: image ZIP -> run inference (re-using your index() flow)
            if has_img:
                try:
                    # --- create per-run folders (same style as index) ---
                    current_datetime = datetime.now()
                    folder_base = 'static/input_img/'
                    ...
                    # --- extract ZIP into 'zip_folder_path' then move images into 'folder_path' exactly like index ---
                    with zipfile.ZipFile(tmp_path, "r") as zf:
                        zf.extractall(zip_folder_path)

                    # if outer folder present, descend into it (skip __MACOSX)
                    exclude_folders = {'__MACOSX'}
                    for f in os.listdir(zip_folder_path):
                        p = os.path.join(zip_folder_path, f)
                        if os.path.isdir(p) and f not in exclude_folders:
                            # move its contents into folder_path
                        ...
                        # otherwise copy files from zip_folder_path root
                        for fn in os.listdir(zip_folder_path):
                            src = os.path.join(zip_folder_path, fn)
                            if os.path.isfile(src):
                                shutil.copy2(src, os.path.join(folder_path, fn))

                    # --- ensure all .jpeg have matching .jgw like your index branch ---
                    ...
                    for image_file in image_files:
                        ...
                        if jgw_file not in file_names:
                            shutil.rmtree(tmp_dir, ignore_errors=True)
                            return JsonResponse({"status": False, "error": f"No corresponding .jgw file found for {image_file}"})

                    # --- load model (pick if not specified) ---
                    try:
                        model_path = _pick_model_file(model_selection, model_name)
                    ...
                    # --- iterate images and run prediction (same as index) ---
                    ...
                    # Clean tif files if present (same helper you have)
                    remove_tif_file(folder_path)

                    # zip original run folder (like index)
            ...
            # If ZIP had neither .pt nor images
            shutil.rmtree(tmp_dir, ignore_errors=True)
            return JsonResponse({"status": False, "message": "ZIP must contain either model (.pt) files or image files."}, status=400)

        # Non-ZIP & non-.pt
        shutil.rmtree(tmp_dir, ignore_errors=True)
        return JsonResponse({"status": False, "message": "Unsupported file type. Upload .pt or .zip."}, status=400)

    # GET: keep original behavior (list models)
    summer_path = f"static/models/germany_summer_ai_model/"
    winter_path = f"static/models/germany_winter_ai_model/"
